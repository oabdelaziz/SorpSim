...
QStringList groupListP;
QStringList listListP;
QStringListModel * listModelP;

ipfixDialog::ipfixDialog(QWidget *parent) :
    QDialog(parent),
...
void ipfixDialog::updateTempSets()
{
    globalpara.resetIfixes('p');
    pSets.clear();
    QSet<Node*> scanSet;
    for(int n = 0; n < globalpara.pGroup.count();n++){
        QSet<Node*> tempSet = globalpara.pGroup.at(n);
...
void ipfixDialog::on_removeButton_clicked()
{
    ...
    if(confirmed)
    {
        QSet<Node*> delSet = globalpara.pGroup.at(ui->listView->currentIndex().row());
        globalpara.pGroup.removeAt(ui->listView->currentIndex().row());
...
void ipfixDialog::on_modifyButton_clicked()
{
    ...
    QStringList currentGroup = listListP.at(index).split("][");
    ...
    if(ui->listView->currentIndex().row()>=0)
    {
        ...
        modifiedString.replace("P#","");
        modifiedString.replace("P-Stream: ","");
        ...
        if(!modifiedString1.isEmpty())
        {
            ...
            QSet<Node*> tempSet = globalpara.pGroup.at(currentGroupIndex);
            globalpara.pGroup.removeOne(tempSet);
            foreach(Node* node,set1){
                tempSet.remove(node);
                node->ipfix=1;
            }
...
bool ipfixDialog::event(QEvent *e)
{
